...
$offices_by_city = get_option("ifchor_offices_by_city", []);
...
<section class="contacts-filter-section" id="our_people_filter">
  <div class="container">
    <form action="" class="contacts-search-form-wrapper" id="our_people_form">
      <div class="contacts-filter-wrapper">
        <div class="contacts-filter-info">
          <h2 class="contacts-filter-info__title"><?php echo esc_html__("Contacts", "ifchor"); ?></h2>
          <!--<p class="contacts-filter-info__content-text"><?php
/*echo esc_html__('Click on the picture to view QR CODE', 'ifchor') */
?></p>-->
        </div>

        <div class="contacts-filter-field">
          <div class="contacts-field-item">
            <select data-placeholder="<?php esc_attr_e("Location", "ifchor"); ?>" name="contact_city" id="contact_city" class="select-field selecttwo-select">
              <option value=""><?php echo esc_attr__("Location", "ifchor"); ?></option>
              <?php
              $loc_city_arr = [];
              foreach ($offices as $loc => $loc_info) {
              	$loc = esc_attr($loc);
              	$loc_arrr = explode("---", $loc);
              	$loc_city = is_array($loc_arrr) ? $loc_arrr[0] : "";
              	$loc_office_id = is_array($loc_arrr) ? $loc_arrr[1] : "";
              	if ($loc_city != "") {
              		$loc_city_arr[] = $loc_city;
              	}
              }

              foreach ($loc_city_arr as $l_city) {
              	echo "<option " . selected($contact_city, $l_city, false) . ' value="' . $l_city . '">' . $l_city . "</option>";
              }
              ?>
          ...
          <div class="contacts-field-item">
            <select data-placeholder="<?php esc_attr_e("Department", "ifchor"); ?>" name="contact_department" id="contact_department" class="select-field selecttwo-select">
              <option value=""><?php echo esc_attr__("Department", "ifchor"); ?></option>
              <?php foreach ($departments_unique as $dept) {
              	$dept = esc_attr($dept);
              	echo "<option " . selected($contact_department, $dept, false) . ' value="' . $dept . '">' . $dept . "</option>";
              } ?>
            </select>
          </div>
          <div class="contacts-field-item contacts-search-form">
            <input type="text" name="contact_name" value="<?php echo $contact_name; ?>" class="contact-search-form" placeholder="NAME" />

            <button type="submit" class="contact-search-btn">
              <svg xmlns="http://www.w3.org/2000/svg" width="18.286" height="18.286" viewBox="0 0 18.286 18.286">
                <g id="Group_6272" data-name="Group 6272" transform="translate(-1057.043 -1583.793)">
                  <g id="Ellipse_37" data-name="Ellipse 37" transform="translate(1057.043 1583.793)" fill="none" stroke="#fff" stroke-width="2">
                    <circle cx="7.843" cy="7.843" r="7.843" stroke="none" />
                    <circle cx="7.843" cy="7.843" r="6.843" fill="none" />
                  </g>
                  <line id="Line_13" data-name="Line 13" x2="4.327" y2="4.327" transform="translate(1070.294 1597.045)" fill="none" stroke="#fff" stroke-width="2" />
                </g>
              </svg>
            </button>
          </div>
    ...
    <div class="contact-items">
      ...
      global $wp_query;
      $args = array_merge($wp_query->query_vars, ["posts_per_page" => 50]);
      query_posts($args);
      ...
      if (have_posts()):
      	while (have_posts()):
      		...
      		$mobile = isset($others["_contact_mobile_number"]) ? $others["_contact_mobile_number"] : "";
      		$id = isset($others["_contact_id"]) ? intval($others["_contact_id"]) : 0;
      		$show_dept = isset($others["_contact_show_dept"]) ? intval($others["_contact_show_dept"]) : 1;

      		$show_phone = isset($others["_contact_show_phone"]) ? intval($others["_contact_show_phone"]) : 1;
      		$office_id = isset($others["_contact_office_id"]) ? intval($others["_contact_office_id"]) : 0;
      		//write_log($office_id);
         ...
         echo '</pre>';*/
      		...
      		$office_info = isset($offices[$city . "---" . $office_id]) ? $offices[$city . "---" . $office_id] : [];
      ...
      <div class="contact-item" data-aos="fade-up">
        ...
        <div class="contact-info-wrapper">
          <div class="contact-info__left">
            ...
            <?php if ($department): ?>
            <p class="contact-info__department">
              <?php echo $department; ?>
            </p>
            <?php endif; ?>

            <?php if ($show_phone && $mobile): ?>
            <p class="contact-info__phone">
              <span class="text-bold"><?php echo esc_html__("M", "ifchor"); ?></span>
            ...
            <div class="contact-info">
              <h4 class="contact-info_title">
                <svg id="Group_2" data-name="Group 2" xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50">
                  <path id="Path_21" data-name="Path 21" d="M-176.074-176.074a21.168,21.168,0,0,1-29.936,0,21.168,21.168,0,0,1,0-29.936,21.167,21.167,0,0,1,29.936,0,21.167,21.167,0,0,1,0,29.936m2.71-32.646a25,25,0,0,0-35.355,0,25,25,0,0,0,0,35.355,25,25,0,0,0,35.355,0,25,25,0,0,0,0-35.355" transform="translate(216.042 216.042)" fill="#fec10d" />
                  <path id="Path_23" data-name="Path 23" d="M-25.667-77.174V-92.2h1.91v-3.572h-8.1V-92.2h1.91v15.029h-1.91V-73.6h8.1v-3.573Z" transform="translate(42.331 109.688)" fill="#fec10d" />
                  <path id="Path_24" data-name="Path 24" d="M-46.921-93.522A11.3,11.3,0,0,1-58.23-104.793a11.3,11.3,0,0,1,11.309-11.271,11.33,11.33,0,0,1,7.352,2.71l.057.049-3.021,3.011-.049-.038a7,7,0,0,0-4.339-1.49,7.049,7.049,0,0,0-7.054,7.029,7.05,7.05,0,0,0,7.054,7.029,7,7,0,0,0,5.1-2.179v-2.415H-46.99v-.011a.214.214,0,0,1-.144-.2.214.214,0,0,1,.063-.151l3.805-3.8h5.633l-.006,8.166-.013.018a11.337,11.337,0,0,1-9.27,4.817" transform="translate(77.905 129.793)" fill="#fec10d" />
                </svg>
                <?php echo $department; ?>
              </h4>
          ...
          <div class="contact-info__middle">
            <?php if ($office_flag != ""): ?>
          ...
          <div class="contact-info__right">
            ...
            <p class="contact-info__phone">
              <span class="contact-icon">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="15.589" height="20.358" viewBox="0 0 15.589 20.358">
                  <defs>
                    <clipPath id="clip-path">
                      <rect id="Rectangle_211" data-name="Rectangle 211" width="15.589" height="20.358" fill="#fff" />
                    </clipPath>
                  </defs>
                  <g id="Group_6259" data-name="Group 6259" transform="translate(0 0)">
                    <g id="Group_6259-2" data-name="Group 6259" transform="translate(0 0)" clip-path="url(#clip-path)">
              ...
              <a href="tel:<?php echo esc_html($office_telephone_number); ?>"><?php echo esc_html($office_telephone_number); ?></a>
            </p>
            <?php endif; ?>
          </div>
      ...
      <?php
      	...
      	//write_log($contact_department);

      	//write_log('no data found but dept is not empty');
      	...
      	//write_log($contact_city_x);
      	//write_log($offices_by_city);
      	//get office info from city
      	//write_log($found_key);
      	//write_log($found_key);
      ...
      <?php if ($contact_department != "") {
      	...
      	if ((is_array($department_info) && sizeof($department_info) > 0) || !empty($department_info)) {
      		...
      		$found_key = array_search($contact_city_x, array_column($offices_by_city, "city"));
      		if ($found_key !== false) {
      			$office_info = $offices_by_city[$found_key + 1];
      		}
      		$office_country = isset($office_info["country"]) ? $office_info["country"] : "";
      		$office_address = isset($office_info["address"]) ? $office_info["address"] : "";
      		$office_telephone_number = isset($office_info["telephone_number"]) ? $office_info["telephone_number"] : "";
      ...
      <div class="contact-item" data-aos="fade-up">
        ...
        <div class="contact-image">
          <div class="contact-main-image">
            <img src="<?php echo esc_url($photo_url); ?>" title="<?php echo esc_attr(the_title()); ?>" alt="<?php echo esc_attr($title); ?>" />
          </div>
        </div>
        <?php endif; ?>
        <div class="contact-info-wrapper">
          ...
          <?php if (is_array($office_info) && sizeof($office_info) > 0): ?>
          <div class="contact-info__right">
            <?php if ($office_flag != ""): ?>
            <div class="contact-info__flag">
              <img src="<?php echo esc_url($office_flag); ?>" alt="<?php echo esc_attr($title); ?>" width="34" height="34" />
      ...
      <p class="text-center"><?php esc_html_e("Sorry, no contacts found, please change your search criteria.", "ifchor"); ?></p>
    ...
    <div class="pagination-wrap d-flex justify-content-center" data-aos="fade-up">
      <?php
      $paginate_args = [
