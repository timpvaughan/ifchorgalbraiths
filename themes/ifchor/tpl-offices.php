    ...
    <div class="row">
      <div class="col-6">
        ...
        <ul id="office_items_filters">
          <li class="<?php echo esc_attr(office_items_filter_active_class($loc, "ALL")); ?>"><a href="<?php echo esc_url(office_items_filter_link("ALL", $dept)); ?>">All</a></li>
          <li class="<?php echo esc_attr(office_items_filter_active_class($loc, "EMEA")); ?>"><a href="<?php echo esc_url(office_items_filter_link("EMEA", $dept)); ?>">EMEA</a></li>
          <li class="<?php echo esc_attr(office_items_filter_active_class($loc, "APAC")); ?>"><a href="<?php echo esc_url(office_items_filter_link("APAC", $dept)); ?>">APAC</a></li>
          <li class="<?php echo esc_attr(office_items_filter_active_class($loc, "NA")); ?>"><a href="<?php echo esc_url(office_items_filter_link("NA", $dept)); ?>">NA</a></li>
        </ul>
      </div>
      <div class="col-6">
        <h3 class="about__subtitle text-end">Departments</h3>
        <?php $offices_arr = ["ALL", "Capesize", "Dry Chartering North America", "Gas - LNG/LPG", "Geared Desk", "IG Sustainability", "Industrial Services and Business Development", "Offshore & Renewables", "Operations", "Panamax", "Panamax", "Research and Advisory", "S&P and Projects", "Tankers"]; ?>
        <div id="dept_items_filters_wrap">
          <select name="dept" id="dept_items_filters">
            <?php foreach ($offices_arr as $office_name) {
    ...
    <div class="row">
      <div class="grid-col-3">
        ...
        foreach ($offices as $office) {
        	...
        	$office_flag_url = "";
        	if ($country != "") {
        		$office_country_sm = str_replace("-", "_", sanitize_title($country)); //var_dump($contacts_content_flags_dir . $office_country_sm . '.png');
        	...
        	if (isset($office["telephone_number"]) && $office["telephone_number"] != "") {
        		echo '<p class="office_telephone_number"><i class="fa-solid fa-circle-phone"></i><a target="_blank" href="tel:' . esc_attr($office["telephone_number"]) . '">' . esc_attr($office["telephone_number"]) . "</a></p>";
        	}
        	if (isset($office["map_url"]) && $office["map_url"] != "") {
        		echo '<p class="office_dir_url"><i class="fa-solid fa-circle-chevron-right"></i><a target="_blank" href="' . esc_url($office["map_url"]) . '">Directions</a></p>';
        	}
        ...
        if ($counter == 0) {
        	echo '<div class="col-md-12"><p class="no_office_found">No results are available, please try again.</p></div>';
        }
        ?>
      </div>
    </div>
