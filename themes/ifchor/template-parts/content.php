<article id="post-<?php the_ID(); ?>" <?php post_class( [ 'post-grid' ] ); ?> data-aos="fade-up">
    ...
    <header class="entry-header">
        ...
        if (!is_wp_error($categories) && is_array($categories) && sizeof($categories) > 0 ) {
            ...
            echo '<a href="' . esc_url( get_term_link( $categories[0] ) ) . '" class="entry-category">' . esc_html( $categories[0]->name ) . '</a>';
            echo '</div>';
        }
		?>
